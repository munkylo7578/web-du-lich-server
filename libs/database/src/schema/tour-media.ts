...
export const tours = pgTable(
  ...
  (table) => [
    check(
      'tours_plans_array_check',
      sql`jsonb_typeof(${table.plans}) = 'array'`,
    ),
    check(
...
export const tourPlans = pgTable(
  'tour_plans',
  {
    id: uuid('id').primaryKey(),
    tourId: uuid('tour_id')
...
export const destinationTranslations = pgTable(
  ...
  (table) => [
    ...
    index('destination_translations_locale_name_idx').on(
      table.locale,
      table.name,
    ),
  ],
...
export const tourDestinations = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('tour_destinations_tour_sort_order_idx').on(
      table.tourId,
      table.sortOrder,
    ),
...
export const images = pgTable(
  ...
  (table) => [
    check(
      'images_url_not_blank_check',
      sql`char_length(trim(${table.url})) > 0`,
    ),
    check(
...
export const tourImages = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('tour_images_tour_sort_order_idx').on(
      table.tourId,
      table.sortOrder,
    ),
...
export const serviceImages = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('service_images_service_sort_order_idx').on(
      table.serviceId,
      table.sortOrder,
    ),
...
export const tourPlanImages = pgTable(
  'tour_plan_images',
  {
    planId: uuid('plan_id')
      .notNull()
  ...
  (table) => [
    ...
    uniqueIndex('tour_plan_images_plan_sort_order_idx').on(
      table.planId,
      table.sortOrder,
    ),
...
export const tourServices = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('tour_services_tour_sort_order_idx').on(
      table.tourId,
      table.sortOrder,
    ),
...
export const tourTranslationsRelations = relations(
  tourTranslations,
  ({ one }) => ({
    tour: one(tours, {
...
export const destinationTranslationsRelations = relations(
...
export const serviceTranslationsRelations = relations(
  serviceTranslations,
  ({ one }) => ({
...
export const tourPlanImagesRelations = relations(tourPlanImages, ({ one }) => ({
  plan: one(tourPlans, {
    fields: [tourPlanImages.planId],
    references: [tourPlans.id],
  }),
  image: one(images, {
    fields: [tourPlanImages.imageId],
    references: [images.id],
  }),
}));
