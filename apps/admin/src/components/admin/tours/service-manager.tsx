...
export function ServiceManager({
  ...
  const [selectedServices, setSelectedServices] =
    useState<AdminService[]>(existingServices);
  const [isPending, startTransition] = useTransition();
  const known = useMemo(
    () =>
      new Map(
        [...existingServices, ...selectedServices, ...results].map((item) => [
          item.serviceId,
          item,
        ]),
      ),
  ...
  const add = (service: AdminService) => {
    ...
    setSelectedServices((current) =>
      current.some((item) => item.serviceId === service.serviceId)
        ? current
        : [...current, service],
    );
    onChange([
      ...value,
      { serviceId: service.serviceId, sortOrder: value.length },
    ]);
    setQuery('');
    setResults([]);
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {query.trim().length >= 2 && (
        <div className="max-h-52 overflow-y-auto rounded-2xl border bg-white p-2">
          {isPending ? (
            <p className="p-4 text-sm">Đang tìm...</p>
          ) : (
            results.map((item) => (
              <button
                key={item.serviceId}
                type="button"
                className="flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2 text-left hover:bg-cyan-50"
                onClick={() => add(item)}
              >
                <span>
                  {
