...
export function TourManagement({ tours, username }: { tours: AdminTour[]; username: string }) {
  ...
  const columns = useMemo(() => [
    ...
    helper.display({ id: "location", header: "Vị trí", cell: ({ row }) => row.original.latitude !== null && row.original.longitude !== null ? <span className="inline-flex items-center gap-1.5 text-sm"><MapPin className="size-3.5 text-muted-foreground" />{row.original.latitude.toFixed(4)}, {row.original.longitude.toFixed(4)}</span> : <span className="text-muted-foreground">Chưa cập nhật</span> }),
